...
module.exports = function(parent, options){
    var verbose = options.verbose,
        routesStack = [{
            path: '/',
            directory: options.routes ? options.routes.replace(/\/$/,'') : '/../routes'
        }],
        apiPath = options.apiPath ? options.apiPath.replace(/\/$/,'') : '/api',
        currentRoute;
    while(routesStack.length > 0) {
        currentRoute = routesStack.pop();
        fs.readdirSync(__dirname + currentRoute.directory).forEach(function(name){
            verbose && console.log('\n   %s:', name);
            ...
            Object.keys(obj).forEach(function(key) {
                var path;
                // "reserved" exports
                if (~['name', 'prefix'].indexOf(key)) continue;
                // route exports
                switch (key) {
                case name:
